...
const BioData = ({ userBio }) => {
    return (
        <div className="rounded-lg border border-gray-900/10 pb-12 space-y-12 bg-slate-800 max-w-4xl my-5 mx-auto px-5 py-10">
            ...
            <UserProfileForm
                fullName={userBio.full_name}
                userName={userBio.user_name}
                email={userBio.email}
                userAvatar={userBio.user_avatar.src}
                coverImg={userBio.cover_img.src}
                userRole={userBio.role_id}
                telNumber={userBio.phone_number}
            />
        </div>
    )
}
